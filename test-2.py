...
def _obtain_input_shape(input_shape,
    ...
    if weights != 'imagenet' and input_shape and len(input_shape) == 3:
        if data_format == '1D':
            default_shape = (input_shape[0], default_size)
...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='', **kwargs):
    ...
    x = layers.Conv1D(block_args.kernel_size,
    ...
    if has_se:
        ...
        se_tensor = layers.Conv1D(num_reduced_filters,
                                  kernal_size=1,
        ...
        se_tensor = layers.Conv1D(filters,
                                  kernal_size=1,
                                  activation='sigmoid',
                                  padding='same',
                                  use_bias=True,
    ...
    # Output phase
    x = layers.Conv1D(block_args.output_filters,
                      kernal_size=1,
                      padding='same',
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
...
def EfficientNet(width_coefficient,
    ...
    # Build stem
    x = img_input
    x = layers.Conv1D(round_filters(32, width_coefficient, depth_divisor),
                      kernal_size=3,
                      strides=2,
                      padding='same',
                      use_bias=False,
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix='block{}a_'.format(idx + 1), **kwargs)
        block_num += 1
