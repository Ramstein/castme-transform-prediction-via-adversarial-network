...
MAX_PATIENCE = 0.2
wav_dir = "../input/freesound-audio-tagging-2019/test/"    "/train/"
...
def main():
    ...
    starttime = time.time()
    p = Pool(2)  # 最大プロセス数=2
    len_list = p.map(get_len, df_test['path'].values)  # get_len for length of the train data
...
def get_mel_batch(df_split):
    ...
    os.makedirs(BATCH_DIR, exist_ok=True)   # BATCH_DIR = "batch"
    p = Pool(2)  # 最大プロセス数=2
    batch_idx = [df_split['batch'].min(), df_split['batch'].max() + 1]
...
def predict_envnet(model, dataloader, activation='sigmoid'):
    ...
    model.eval()
    for i, input in enumerate(dataloader):
        input = input.cuda(non_blocking=True)   # async is now a reserved word in Python >= 3.7 so use non_blocking instead
