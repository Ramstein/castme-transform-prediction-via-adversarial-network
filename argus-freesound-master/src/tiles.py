...
class ImageSlicer:
    ...
    def __init__(self, image_shape, tile_size, tile_step=0, image_margin=0, weight='mean'):
        ...
        for y in range(0, self.image_height + self.margin_top + self.margin_bottom - self.tile_size[0] + 1,
                       self.tile_step[0]):
            for x in range(0, self.image_width + self.margin_left + self.margin_right - self.tile_size[1] + 1,
                           self.tile_step[1]):
    ...
    def split(self, image, border_type=cv2.BORDER_CONSTANT, value=0):
        ...
        image = cv2.copyMakeBorder(image, self.margin_top, self.margin_bottom, self.margin_left, self.margin_right,
    ...
    def crop_to_orignal_size(self, image):
        ...
        crop = image[self.margin_top:self.image_height + self.margin_top,
